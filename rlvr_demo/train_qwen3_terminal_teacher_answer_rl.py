"""AReaL teacher-answer RL entry point for terminal-agent command payloads."""
...
from rlvr_demo.terminal_agent_data import get_terminal_teacher_answer_rl_dataset
...
def main(args: list[str]) -> None:
    ...
    with PPOTrainer(config, train_dataset=train_dataset, valid_dataset=valid_dataset) as trainer:
        trainer.train(
            workflow="rlvr_demo.terminal_agent_rl.TerminalTeacherAnswerRLWorkflow",
            workflow_kwargs=workflow_kwargs,
            eval_workflow="rlvr_demo.terminal_agent_rl.TerminalTeacherAnswerRLWorkflow",
            eval_workflow_kwargs=eval_workflow_kwargs,
            rollout_postprocess_fn=(
                "rlvr_demo.terminal_agent_rl.teacher_answer_reward_postprocess"
            ),
        )
