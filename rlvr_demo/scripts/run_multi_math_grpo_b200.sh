...
CONFIG="${1:-$REPO_ROOT/rlvr_demo/configs/qwen3_06b_multi_math_grpo_b200_250.yaml}"
if [[ $# -gt 0 ]]; then
  shift
fi

cd "$REPO_ROOT"
exec "$REPO_ROOT/.venv/bin/python" -m rlvr_demo.train_qwen3_multi_math --config "$CONFIG" "$@"
