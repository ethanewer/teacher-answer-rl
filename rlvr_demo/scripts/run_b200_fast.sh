...
CONFIG="${1:-$REPO_ROOT/rlvr_demo/configs/qwen3_06b_gsm8k_areal_megatron_b200_fast.yaml}"
if [[ $# -gt 0 ]]; then
  shift
fi

cd "$REPO_ROOT"
exec "$REPO_ROOT/.venv/bin/python" -m rlvr_demo.train_qwen3_gsm8k --config "$CONFIG" "$@"
