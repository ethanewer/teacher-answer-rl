...
CONFIG="${1:-$REPO_ROOT/rlvr_demo/configs/qwen3_4b_thinking_terminus_tool_grpo_smoke_h200.yaml}"
if [[ $# -gt 0 ]]; then
  shift
fi
...
cd "$REPO_ROOT"
exec "$AREAL_VENV/bin/python" -m rlvr_demo.train_qwen3_terminal_tool_grpo --config "$CONFIG" "$@"
