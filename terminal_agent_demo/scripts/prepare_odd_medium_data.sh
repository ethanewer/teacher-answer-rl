...
OUTPUT="${1:-/wbl-fast/usrs/ee/teacher-answer-rl/areal_runs/terminal-agent-demo/data/skill_based_medium.odd_original.terminus_tool.jsonl}"
...
"$AREAL_VENV/bin/python" -m terminal_agent_demo.terminus_tool_calling inspect-converted \
  ...
  --output "$INSPECT" \
  -n "${INSPECT_N:-5}"
echo "Converted odd-index data: $OUTPUT"
